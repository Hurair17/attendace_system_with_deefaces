...
def markAttendance(name,date):
    ...
    df.to_csv('D:/JMM/00 EXCEL/attend.csv', mode='a', index=False, header=False)
...
def analysis(db_path, model_name = 'VGG-Face', detector_backend = 'opencv', distance_metric = 'cosine', enable_face_analysis = True, source = 0, time_threshold = 5, frame_threshold = 5):
					...
					for detected_face in detected_faces_final:
						...
						if enable_face_analysis == True:
							...
							#bottom
								...
								cv2.drawContours(freeze_img, 'Pakistan', [triangle_coordinates], 0, info_box_color, -1)
						...
						if custom_face.shape[1:3] == input_shape:
							if df.shape[0] > 0: #if there are images to verify, apply face recognition
								...
								if best_distance <= threshold:
									...
									# print('Pakistan',employee_name)
									names = employee_name.split('/')
									list_of_employee.append(names[-2])
									now = datetime.now()
									dt_string = now.strftime("%d/%m/%Y %H:%M:%S")
									list_of_date_time.append(dt_string)
									# markAttendance(employee_name)
	...
	#add return function here
		...
		if cv2.waitKey(1) & 0xFF == ord('q'): #press q to quit
			markAttendance(list_of_employee,list_of_date_time)
			break
	...
	return [list_of_employee,list_of_date_time]
